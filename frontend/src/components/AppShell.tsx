...
export default function AppShell() {
  ...
  return (
    <div className="paper-noise min-h-svh bg-[var(--linen)] text-[var(--ink)]" data-testid="journal-app-shell">
      ...
      <Onboarding />
      {!online && <div className="theme-card-overlay fixed bottom-5 left-1/2 z-40 -translate-x-1/2 rounded-full border border-[var(--line-strong)] px-4 py-2 text-xs font-semibold whitespace-nowrap max-sm:left-4 max-sm:right-4 max-sm:translate-x-0 max-sm:whitespace-normal max-sm:text-center text-[var(--ink-soft)] shadow-[var(--shadow-md)] backdrop-blur-md" role="status" data-testid="offline-pill">{queued > 0 ? `You’re offline — ${queued} ${queued === 1 ? "page" : "pages"} waiting to save.` : "You’re offline — new words will keep, and save when you return."}</div>}
